...
INPUT_FILE = Path(
    r"C:\Users\JurisFedotovs\Desktop\RTU\MASTER\Āgenskalna_klīnika_sarunas\EXPERIMENT\CODE_BASE\DATA\04_lv_sarunas_cleaned.csv"
)
OUTPUT_FILE = INPUT_FILE.with_name("05_lv_freaquencies.csv")
WORDCLOUD_PNG = INPUT_FILE.with_name("05_lv_wordcloud.png")
...
def simple_tokenize(text):
    """Extract Latvian words before lemmatization."""
    if pd.isna(text):
        return []
    return LATVIAN_WORD_RE.findall(str(text).lower())

def lemmatize_words(words):
    """Lemmatize a list of Latvian words using Stanza."""
    if not words:
        return []
